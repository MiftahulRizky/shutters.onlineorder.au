...
function bindBlindType(designId) {
    ...
    bindColourType(blindtype.value);

    if (!designId) return;

    var listData = { type: "BlindType", designtype: designId };
    $.ajax({
        type: "POST",
...
function bindColourType(blindId) {
    ...
    bindComponentForm(colourtype.value);

    if (!blindId) return;

    var listData = { type: "ColourType", blindtype: blindId, tubetype: "N/A", controltype: "N/A" };
...
function bindComponent(category) {
    ...
    const componentOptions = {
        "Louvres": [
            "63mm Louvre", "89mm Louvre", "Standard Louvre Pin", "Louvre Repair Pin"
        ],
        "Framing | Hinged": [
            "Beaded L Frame 49mm x 22mm", "Insert L Frame 49mm x 22mm", "Insert L Frame Extension 38mm x 13mm", "Small Bullnose Z Frame", "Large Bullnose Z Frame", "U Channel - Top", "U Channel - Bottom"
        ],
        "Framing | Bi-fold or Sliding": [
            "101.6mm x 19mm Header", "161.6mm x 19mm Header", "201.6mm x 19mm Header",
            "100mm x 19mm Sideboard/Bottom Board", "160mm x 19mm Sideboard/Bottom Board",
            "200mm x 19mm Sideboard/Bottom Board", "63.5mm x 9.5mm Fascia",
            "100mm x 9.5mm Fascia", "140mm x 9.5mm Fascia",
            "31.8mm x 9.5mm Side Fascia", "25mm x 44.5mm Header Support Strip",
            "19mm x 19mm Header Support Strip", "19mm x 25mm Light Strip",
            "25mm x 25mm Light Strip", "30mm x 25mm Light Strip", "40mm x 25mm Light Strip"
        ],
        "Framing | Fixed": [ "U Channel - Top", "U Channel - Bottom" ],
        "Posts": [
            "25mm T Post", "90° Corner Post", "135° Bay Post", "T Post Mounting Bracket"
        ],
        "Extrusion": [
            "19mm x 19mm Light Block", "9.5mm x 31.8mm Frame Buildout", "25mm x 44.5mm Frame Buildout"
        ],
        "Hinges": [
            "76mm Non-Mortise Hinge", "76mm Rabbet Hinge", "Hinge Spacer"
        ],
        "Magnets and Strikers": [
            "Magnet", "Standard Striker Plate", "L Shape Striker Plate"
        ],
        "Track Hardware": [
            "Top Track", "Bottom M Track", "Bottom U Track", "Top Pivot Set",
            "Bottom Pivot (Incl Bracket)", "Wheel Carrier", "Spring Loaded Guide",
            "Adjustment Spanner", "Track Stop"
        ],
        "Misc": ["Hoffman Key"]
    };
...
function bindColour(category, component) {
    ...
    const colourOptions = {
        "Louvres": component === "Standard Louvre Pin" || component === "Louvre Repair Pin"
...
function isColourVisible(category, component) {
    const colourCategories = new Set([
        "Louvres", "Framing | Hinged", "Framing | Bi-fold or Sliding",
        "Framing | Fixed", "Posts", "Extrusion", "Hinges",
        "Magnets and Strikers", "Misc"
    ]);
    const hiddenComponents = new Set(["Post Mounting Brack", "Hinge Spacer", "Magnet"]);
...
function isLengthVisible(category, component) {
    const lengthCategories = new Set([
        "Louvres", "Framing | Hinged", "Framing | Bi-fold or Sliding",
        "Framing | Fixed", "Extrusion", "Track Hardware"
    ]);
    const hiddenComponents = new Set([
        "Standard Louvre Pin", "Louvre Repair Pin", "Post Mounting Bracket",
        "Top Pivot Set", "Bottom Pivot (Incl Bracket)", "Wheel Carrier",
        "Spring Loaded Guide", "Adjustment Spanner", "Track Stop"
    ]);
...
function proccess() {
    ...
    const fields = [
        "blindtype", "colourtype", "qty", "category", "component",
        "colour", "length", "notes", "markup"
    ];
...
function bindItemOrder(itemId) {
    $.ajax({
        ...
        success: function (response) {
            ...
            var blindtype = itemData["BlindType"];
            var colourtype = itemData["ColourType"];
            var category = itemData["Category"];
            var component = itemData["Component"];
            ...
            setTimeout(function () { setFormValues(itemData); }, 2000);
            setTimeout(function () { bindComponentForm(colourtype); }, 2300);
        },
        error: function (xhr, status, error) {
